...
const names = ['gto-preflop-charts-all', 'gto-cache-preflop-chart', 'bencb-preflop-charts', 'tournament-flop-cbet'];
...
function fixture(t) {
  ...
  copyFileSync(fileURLToPath(new URL('./charts-crypto.mjs', import.meta.url)), join(root, 'scripts/charts-crypto.mjs'));
  const paths = [...names.map(name => join(publicDir, `${name}.json`)), join(root, 'scripts/data/flop-board-textures.json')];
...
test('corrupt last ciphertext preserves every existing plaintext', t => {
  ...
  const path = f.paths.at(-1) + '.enc'; const bytes = readFileSync(path);
...
test('invalid decrypted JSON does not replace plaintext', t => {
  const f = fixture(t); assert.equal(f.run('encrypt').status, 0);
  const before = f.snapshot('');
  const cipher = spawnSync('openssl', ['enc', '-aes-256-cbc', '-e', '-salt', '-pbkdf2', '-out', f.paths.at(-1) + '.enc', '-pass', 'env:DATA_KEY'], { env: { ...process.env, DATA_KEY: testKey }, input: 'not json' });
...
test('invalid last plaintext preserves all existing ciphertext', t => {
  ...
  writeFileSync(f.paths.at(-1), 'not json');
