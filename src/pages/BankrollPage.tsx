...
export function BankrollPage() {
	...
	useEffect(() => {
		...
		fetchBankrollSessions()
			.then((store) => {
				if (!cancelled) setSessions(dedupeSessions(flattenStore(store)));
			})
			.catch(() => {
	...
	async function onFiles(e: ChangeEvent<HTMLInputElement>) {
		...
		const importedTicketPrices = mergeTicketPrices(
			parsedFiles.map(({ parsed }) => extractTicketPrices(parsed)),
		);
		...
		<div className="space-y-4">
			...
			{hasData && (
				<>
					...
					<Section
						title="Records"
						right={`${listedSessions.length} / ${sessions.length} sessions`}
					>
							...
							<table className="min-w-full text-left text-xs">
								...
								<tbody className="divide-y divide-gray-800">
									{listedSessions.map((session) => {
										...
										const editingMissingTicketPrice = Boolean(
											isEditing &&
												session.isTicket &&
												draft.ticketPrice.trim() === "",
										);
										return (
											<tr
											...
											>
												<td className="whitespace-nowrap px-2 py-2 align-top">
													{isEditing ? (
														<input
															...
															onChange={(e) =>
																setDraft({ ...draft, datetime: e.target.value })
															}
												...
												<td className="whitespace-nowrap px-2 py-2 align-top">
													<span
														className={`rounded px-2 py-1 text-[11px] uppercase ${missingTicketPrice ? "bg-red-900/70 text-red-100 ring-1 ring-red-500/70" : "bg-gray-800 text-gray-300"}`}
													>
												...
												<td className="px-2 py-2 align-top">
													{isEditing ? (
														...
														<div
															className="max-w-80 truncate text-gray-200"
															...
															>
																{missingTicketPrice ? (
																	...
																	<span className="flex flex-col items-end gap-0.5">
																		<span>
																			Cost {formatUsd(session.buyIn ?? 0)}
																		</span>
																		<span className="text-green-300">
																			Ticket +
																			{formatUsd(session.ticketPrice ?? 0)}
																		</span>
																	</span>
																) : (
												...
												<td
													className={`whitespace-nowrap px-2 py-2 text-right align-top font-semibold ${session.profit >= 0 ? "text-green-400" : "text-red-400"}`}
												>
													...
													{isEditing ? (
														<div className="flex justify-end gap-2">
															...
															<button
																onClick={cancelEdit}
																disabled={syncing}
																className="rounded border border-gray-700 bg-gray-800 px-2 py-1 text-gray-300 hover:text-white disabled:opacity-50"
															>
																취소
															</button>
														</div>
														...
														<div className="flex justify-end gap-2">
															...
															<button
																...
																disabled={syncing}
																className="rounded border border-red-900/70 bg-red-950/40 px-2 py-1 text-red-300 hover:text-red-200 disabled:opacity-50"
															>
																삭제
															</button>
														</div>
...
function collectTicketPrices(
	...
	for (const [id, row] of ticketRows) {
		...
		if (Number.isFinite(parsedPrice) && parsedPrice >= 0) {
			...
			window.alert(
				"0 이상의 숫자로 입력해주세요. 이 기록은 티켓 가격 없이 저장됩니다.",
			);
		}
	}
...
function mergeTicketPrices(
	priceMaps: Record<string, number>[],
): Record<string, number> {
...
function draftToSession(
	...
	if (winLoss === null)
		return { ok: false, error: "Win/Loss는 숫자로 입력해주세요." };
	...
	const rank =
		draft.rank.trim() === "" ? undefined : parseDraftInteger(draft.rank);
	if (buyIn === null || buyIn < 0)
		return { ok: false, error: "Cost는 0 이상의 숫자로 입력해주세요." };
	if (ticketPrice !== null && ticketPrice < 0)
		return { ok: false, error: "Cost는 0 이상의 숫자로 입력해주세요." };
	if (entries === null || entries < 1)
		return { ok: false, error: "Entries는 1 이상의 정수로 입력해주세요." };
	if (rank === null || (rank !== undefined && rank < 1))
		return {
			ok: false,
			error: "Rank는 비워두거나 1 이상의 정수로 입력해주세요.",
		};
	const resolvedTicketPrice = original.isTicket
		? (ticketPrice ?? undefined)
		: original.ticketPrice;
...
function fromDateTimeInput(datetime: string): string {
	...
	return trimmed.includes("T")
		? `${trimmed.replace("T", " ")}:00`.slice(0, 19)
		: trimmed;
}
